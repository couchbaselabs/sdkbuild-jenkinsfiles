                    ...
                    stage('build') {
                        steps {
                            dir("ws_win_${MSVS.replaceAll(' ', '_')}_${ARCH}/build") {
                                ...
                                bat('dir "%ProgramFiles%"')
                                bat('dir "%ProgramFiles%\\OpenSSL-Win64\\"')
                                bat('dir "%ProgramFiles(x86)%"')
                                bat('dir "%ProgramFiles(x86)%\\OpenSSL-Win32\\"')
                                bat("cmake -G\"Visual Studio ${MSVS}\" -A ${ARCH} ${TLS.toBoolean() ? (ARCH == "x64" ? '-DOPENSSL_ROOT_DIR="%ProgramFiles%\\OpenSSL-Win64"' : '-DOPENSSL_ROOT_DIR="%ProgramFiles(x86)%\\OpenSSL-Win32"') : '-DLCB_NO_SSL=1'} ..\\libcouchbase")
                                bat('cmake --build .')
                            }
                        }
                    }

    ...
    for (cluster_version in CLUSTER_VERSIONS) {
        stage("combintation-test ${platform}/${cluster_version}") {
            ...
            try {
                ...
                shWithEcho("sed -i -e 's/hostname.*/hostname\": \"${ip}\",/' Couchbase.Extensions/tests/Couchbase.Extensions.Caching.IntegrationTests/config.json")

                // run integration tests
                shWithEcho("deps/dotnet-core-sdk-${DOTNET_SDK_VERSION}/dotnet test Couchbase.Extensions/tests/Couchbase.Extensions.Caching.IntegrationTests/Couchbase.Extensions.Caching.IntegrationTests.csproj")
            }
            finally {
                if (clusterId != null) {

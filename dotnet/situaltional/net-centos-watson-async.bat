...
git clone http://Oferya:oferhub11@github.com/couchbase/sdkd-net
cd %SDKD-NET%
...
call packages\sdkdclient\bin\brun.bat --bucket-password="" --rebound %rebound% -A S3Creds_tmp -I cluster_config.ini -I sdkd-async.args --testsuite-variants "HYBRID" --install-skip false --install-version %cluster_version% --install-url %url% --cluster-useSSL=%useSSL% -d all:debug

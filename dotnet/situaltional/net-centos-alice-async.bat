...
echo STEP4 - now running: git clone http://Oferya:oferhub11@github.com/couchbase/couchbase-net-client.git --single-branch --branch %client_version%
...
# Analytics #
call packages\sdkdclient\bin\brun.bat --rebound %rebound% -A S3Creds_tmp -I analytics_config.ini -I sdkd-async.args --bucket-password="password" --testsuite-variants "CBAS" --install-url %url% --install-skip false --install-version %cluster_version% --cluster-useSSL=%useSSL% -d all:debug

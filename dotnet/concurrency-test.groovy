                ...
                stage("Get Ready") {
                    ...
                    parallel {
                        stage("Install Dependencies") {
                            environment {
                                DOTNET_ROOT="$WORKSPACE/deps/dotnet-core-sdk-all/"
                                PATH="$DOTNET_ROOT:$HOME/.dotnet/tools:$WORKSPACE/deps/dotnet-core-sdk-all/host/fxr/6.0.1:$PATH"
                            }
                            steps {
                                installSdksForPlatform("linux", DOTNET_SDK_VERSIONS)
                                dotNetWithEcho("linux", "--info")
                                dotNetWithEcho("linux", "tool install -g dotnet-counters || echo 'dotnet-counters already installed'")
                                dotNetWithEcho("linux", "counters -h")
                            }
                        }
                        stage("Start Cluster") {
                            steps {
                        ...
                        stage("Start Concurrency Tests") {
                            steps {
                                script {
                                    ...
                                    def dotNetCmd = dotNetCmd = getDotnetCmd("linux")
                                    env.CONNECTION_STRING = "couchbase://${CURRENT_CLUSTER_IP}"
                                    dotNetWithEcho("linux", "run --project couchbase-net-client/tests/Couchbase.ConcurrencyTests/Couchbase.ConcurrencyTests.csproj ${COMMAND_LINE_OPTIONS} --connectionString 'couchbase://${CURRENT_CLUSTER_IP}'")
                                 }
                            }
                        }
                        stage("Collect Counters") {
                            environment {
                                DOTNET_ROOT="$WORKSPACE/deps/dotnet-core-sdk-all/"
                                PATH="$DOTNET_ROOT:$HOME/.dotnet/tools:$WORKSPACE/deps/dotnet-core-sdk-all/host/fxr/6.0.1:$PATH"
                            }

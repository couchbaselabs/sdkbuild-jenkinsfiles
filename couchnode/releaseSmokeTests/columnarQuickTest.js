...
async function runSmokeTest() {
  ...
  try {
    ...
    const credential = new columnar.Credential(
      testConfig.username,
      testConfig.password
    )
    options = {
    ...
    const cluster = columnar.createInstance(
      testConfig.connStr,
      credential,
      options
    )
    ...
    let qs = `SELECT * FROM \`${testConfig.databaseName}\`.\`${testConfig.scopeName}\`.\`${testConfig.collectionName}\` LIMIT 2;`

...
#
# `npm ci` clears the directory itself, but it does that inside the same reify pass that
# extracts the new tree. In a REUSED workspace where anything still holds a handle (on
# Windows the search indexer, AV, or a stray node.exe from a previous build) a directory
# removal fails while the extraction into it goes ahead, and npm reports the result as a
# flood of `npm warn tar ENOENT: Cannot cd into '.../node_modules/<pkg>'` with packages
# missing from the installed tree. Clearing it first means npm only ever extracts onto
# empty ground, and a tree left by a different commit can never survive into this build.
#
# Best effort by design: if the removal cannot finish we are no worse off than npm's own,
# so this warns and lets the install proceed rather than failing the stage here.
_clear_node_modules() {
...
task_test() {
    ...
    "${NPM_BIN}" install --no-save --ignore-scripts mocha-multi-reporters || true
    ...
    local cmd rc=0 injected_reporter=0
    for cmd in "${cmds[@]}"; do
        local run_cmd="${cmd}"
    ...
    if [[ "${rc}" -eq 0 && "${injected_reporter}" -eq 1 && ! -f xunit.xml ]]; then

...
from acouchbase_columnar.options import ClusterOptions, SecurityOptions
...
async def cluster_test(config: TestConfig) -> None:
...
async def scope_test(config: TestConfig) -> None:
    connstr = config.get_connstr()
    if config.nonprod is True:
        from couchbase_columnar.common.core._certificates import _Certificates
        sec_opts = SecurityOptions.trust_only_certificates(_Certificates.get_nonprod_certificates())
    ...
    cred = Credential.from_username_and_password(config.username, config.password)
    scope = AsyncCluster.create_instance(connstr, cred, opts).database(config.database).scope(config.scope)
...
async def main() -> None:
    config = get_config()
    await cluster_test(config)
    await scope_test(config)

...
The accompanying strip + separate-debug-wheel split (objcopy --only-keep-debug /
--strip-unneeded / --add-gnu-debuglink) is orchestrated by tasks.sh as the rest of the
repair command.

No-op when OpenSSL is not in use (boringssl builds), so it is safe to wire unconditionally.
"""
...
def _whitelist_openssl() -> None:
    ...
    Reads PYCBC_USE_OPENSSL / PYCBC_OPENSSL_VERSION, which cibuildwheel re-exports into
    the build via CIBW_ENVIRONMENT. BEFORE_ALL env does not carry into the build.
    """

...
"""
        ...
        {
            cmd=("""
export PYENV_ROOT=${WORKSPACE}/deps/pyenv
export PYENV_DEBUG="True"
curl https://pyenv.run | bash 
echo "export PATH=\\"${WORKSPACE}/bin:\\$PATH\\"">>${WORKSPACE}/deps/.bashrc
echo "eval \\"\\\$(pyenv init -)\\"">>${WORKSPACE}/deps/.bashrc
bash -c "source ${WORKSPACE}/deps/.bashrc
...
def pythonWithEcho(String command, python_version=null, platform="linux")
{
    ...
    {
        command="""
export PATH="${WORKSPACE}/deps/pyenv/versions/${python_version}/bin:$PATH"
bash -c "source ${WORKSPACE}/deps/.bashrc
        ...
        if (isWindows(platform)) {
            ...
            dir("couchbase-python-client") {
                ...
                withEnv(envStr+["CPATH=${LCB_INC}", "LIBRARY_PATH=${LCB_LIB}"]) {
                    ...
                    batWithEcho("python -m pip install wheel")
                }
